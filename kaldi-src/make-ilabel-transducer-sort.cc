...
int main(int argc, char *argv[]) {
  try {
	...
	po.Register("sort-disambig", &sort_disambig, "sort disambig in new ilabel tail");
    po.Read(argc, argv);
	...
	// sort ,make disambig is end. add by hubo
	if(sort_disambig)
	{
	    std::vector<int32> sort_old2new_mapping;
		std::vector<std::vector<int32> > sort_new_ilabels;
		sort_new_ilabels.resize(new_ilabels.size());
		sort_old2new_mapping.resize(new_ilabels.size());
		int32 dis_id=sort_new_ilabels.size()-1;
		int32 cdphone_id = 0;
		for (size_t new_idx = 0; new_idx < new_ilabels.size(); new_idx++)
		{
			if(new_ilabels[new_idx].size() == 1 && new_ilabels[new_idx][0] <= 0)
			{
				sort_new_ilabels[dis_id] = new_ilabels[new_idx];
				sort_old2new_mapping[new_idx] = dis_id;
				dis_id--;
			}
			else
			{
				sort_new_ilabels[cdphone_id] = new_ilabels[new_idx];
				sort_old2new_mapping[new_idx] = cdphone_id;
				cdphone_id++;
			}
		}
		if(cdphone_id - dis_id != 1)
		{
			KALDI_ERR << "sort new ibalels error " << dis_id << " - " << cdphone_id << "!= 1";
		}
		// modey old2new_mapping
		for (size_t old_idx = 0; old_idx < old2new_mapping.size(); old_idx++)
		{
			int32 sort_id = old2new_mapping[old_idx];
			old2new_mapping[old_idx] = sort_old2new_mapping[sort_id];
		}
		new_ilabels = sort_new_ilabels;
	}// end 
